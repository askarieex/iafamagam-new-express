...
export default function LedgerSnapshots() {
    ...
    const fetchPeriodStatuses = async () => {
        try {
            // Get period statuses for the entire selected year
            ...
            // Use the period statuses directly from the API
            let statuses = {};
            if (periodResponse.data.success && periodResponse.data.data.periods) {
                statuses = periodResponse.data.data.periods;
                console.log(`Loaded period statuses for ${selectedYear}:`, statuses);
            } else {
                // Default fallback: set all periods to closed
                for (let month = 1; month <= 12; month++) {
                    statuses[month] = false;
                }
                
                // Only mark current month as open if we're viewing the current year and no periods exist
                const currentDate = new Date();
                const currentMonth = currentDate.getMonth() + 1;
                const currentYear = currentDate.getFullYear();
                
                if (selectedYear === currentYear) {
                    console.log(`No period data found, marking current month (${currentMonth}) as open for year ${currentYear}`);
